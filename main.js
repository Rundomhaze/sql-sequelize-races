...
async function createTables() {
  ...
  await sequelize.query(
    'CREATE TABLE races(id serial PRIMARY KEY, name text, location text, created_at date, updated_at date);'
  );
  await sequelize.query(
  ...
  await sequelize.query(
    `INSERT INTO races (name, location, created_at, updated_at) VALUES ('Race One', 'Dubai', NOW(), NOW()), ('Race Two', 'OAЭ', NOW(), NOW()), ('Race Three', 'Egypt', NOW(), NOW())`
  );
  await sequelize.query(
